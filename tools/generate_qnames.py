...
def write_names_file(names, func_name, fd):
    fd.write('# generated by "generate_qnames"\n' )
    fd.write(f'from .namespaces import {func_name}\n')
    fd.write('')
    names_list = [n.localname for n in names.values()]
    names_list.sort()
    for name in names_list:
        fd.write(f"{name} = {func_name}('{name}')\n")
...
if __name__ == '__main__':
    ...
    with open('../sdc11073/pm_qnames.py', 'w') as fd:
        write_names_file(names, 'domTag', fd)
    ...
    with open('../sdc11073/msg_qnames.py', 'w') as fd:
        write_names_file(names, 'msgTag', fd)
    ...
    with open('../sdc11073/ext_qnames.py', 'w') as fd:
        write_names_file(names, 'extTag', fd)
