...
class _OperationsWorker(threading.Thread):
    ...
    def __init__(self, subscriptionsmgr, mdib, log_prefix):
        ...
        '''
        super(_OperationsWorker, self).__init__(name='DeviceOperationsWorker')
        self.daemon = True
    ...
    def run(self):
        while True:
            try:
                ...
                self._subscriptionsmgr.notifyOperation(self._mdib.sequenceId, self._mdib.mdibVersion, tr_id, operation.handle, pmtypes.InvocationState.WAIT)
                time.sleep(0.001) # not really necessary, but in real world there might also be some delay.
                self._subscriptionsmgr.notifyOperation(self._mdib.sequenceId, self._mdib.mdibVersion, tr_id, operation.handle, pmtypes.InvocationState.START)
                try:
                    operation.executeOperation (request)
                    self._logger.info('{}: successfully finished operation "{}"', operation.__class__.__name__, operation.handle)
                    self._subscriptionsmgr.notifyOperation(self._mdib.sequenceId, self._mdib.mdibVersion, tr_id, operation.handle,  pmtypes.InvocationState.FINISHED)
                except Exception as ex:
                    self._logger.info('{}: error executing operation "{}": {}',  operation.__class__.__name__, operation.handle, traceback.format_exc())
                    self._subscriptionsmgr.notifyOperation(self._mdib.sequenceId, self._mdib.mdibVersion, tr_id, operation.handle,  pmtypes.InvocationState.FAILED, error='Oth', errorMessage=repr(ex))
            except Exception as ex:
                self._logger.error('{}: unexpected error while handling operation "{}": {}', operation.__class__.__name__, operation.handle, traceback.format_exc())
...
class OperationDefinition(object):
    ...
    def _initOperationTargetContainer(self):
        ...
        self._operationTargetContainer = self._mdib.states.descriptorHandle.getOne(self._operationTargetHandle, allowNone=True)  #pylint:disable=protected-access
        if self._operationTargetContainer is not None:
            self._logger.info('operation target state for operation "{}" is already present, re-using it'.format(self._operationTargetHandle))
        else:
            self._operationTargetContainer = self._mdib.mkStateContainerFromDescriptor(operationTargetDescriptor)  #pylint:disable=protected-access
            self._operationTargetContainer.updateNode()
            self._logger.info('creating {} DescriptorHandle = {}', self._operationTargetContainer.__class__.__name__, self._operationTargetHandle)
            if self._operationTargetContainer is not None:
    ...
    def __str__(self):
        return '{} handle={} operationTarget={}'.format(self.__class__.__name__, self._handle, self._operationTargetHandle)
...
class SetStringOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, initialValue=None, codedValue=None):
        super(SetStringOperation, self).__init__(handle=handle,
...
class SetValueOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, initialValue=None, codedValue=None):
        super(SetValueOperation, self).__init__(handle=handle,
                                                operationTarget=operationTarget,
...
class SetContextStateOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, codedValue=None):
        super(SetContextStateOperation, self).__init__(handle,
                                                       operationTarget,
...
class ActivateOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, codedValue=None, safetyReq=None):
        super(ActivateOperation, self).__init__(handle=handle,
                                                operationTarget=operationTarget,
...
class SetAlertStateOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, codedValue=None, safetyReq=None, log_prefix=None):
        super(SetAlertStateOperation, self).__init__(handle=handle,
                                                    operationTarget=operationTarget,
    ...
    def executeOperation(self, request):
        ...
        if len(proposedAlertStateNodes) > 1: #schema allows exactly one ProposedAlertState:
            raise ValueError('only one ProposedAlertState argument allowed, found {}'.format(len(proposedAlertStateNodes)))
...
class SetComponentStateOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, codedValue=None, safetyReq=None, log_prefix=None):
        super(SetComponentStateOperation, self).__init__(handle=handle,
                                                operationTarget=operationTarget,
...
class SetMetricStateOperation(OperationDefinition):
    ...
    def __init__(self, handle, operationTarget, codedValue=None, safetyReq=None, log_prefix=None):
        super(SetMetricStateOperation, self).__init__(handle=handle,
                                                      operationTarget=operationTarget,
