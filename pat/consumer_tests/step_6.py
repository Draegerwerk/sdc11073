...
def test_6b(consumer: SdcConsumer) -> bool:  # noqa: C901
    ...
    set_context_state: descriptorcontainers.SetContextStateOperationDescriptorContainer = consumer.mdib.descriptions.handle.get_one('set_context_0.sco.mds_0')
    step = f'{__STEP__}b'
    ...
    for p in patient_context_descriptors:
        for association in [
            ...
            event = threading.Event()
            observer = functools.partial(_on_context_state, event=event, descriptor_handle=p.Handle, pat=pat, association_to_listen=association)
...
def test_6c(consumer: SdcConsumer) -> bool:
    ...
    operations_: Sequence[descriptorcontainers.SetValueOperationDescriptorContainer] = consumer.mdib.descriptions.NODETYPE.get(pm_qnames.SetValueOperationDescriptor)
    if not operations_:
        logger.error('The reference provider provides no set value operations', extra={'step': step})
        return False
    test_results: list[bool] = []
    for operation in operations_:
        state: statecontainers.SetValueOperationStateContainer = consumer.mdib.states.descriptor_handle.get_one(operation.Handle)
        fut: Future[operations.OperationResult] = set_service.set_numeric_value(operation.Handle, state.AllowedRange[0].Lower)
        try:
...
def test_6e(consumer: SdcConsumer) -> bool:  # noqa: C901, PLR0912
    ...
    operations_: Sequence[descriptorcontainers.SetMetricStateOperationDescriptorContainer] = consumer.mdib.descriptions.NODETYPE.get(pm_qnames.SetMetricStateOperationDescriptor)
    if not operations_:
    ...
    for operation in operations_:
        ...
        # TODO: check if metric values were actually changed in mdib after operation finished https://github.com/Draegerwerk/sdc11073/pull/436#discussion_r2470759128
...
def test_6f(consumer: SdcConsumer) -> bool:
    ...
    try:
        activate_operation: descriptorcontainers.ActivateOperationDescriptorContainer = consumer.mdib.descriptions.handle.get_one(activate_operation_handle)
    except KeyError:
