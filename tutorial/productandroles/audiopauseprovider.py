...
class GenericAudioPauseProvider(RoleProvider):
    ...
    def make_operation_instance(
        ...
        if operation_descriptor_container.Type is not None:
            if _coded_value_comparator(
                operation_descriptor_container.Type,
                NomenclatureCodes.MDC_OP_SET_ALL_ALARMS_AUDIO_PAUSE,
            ):
                self._logger.debug(
                    'instantiating "set audio pause" operation from existing descriptor handle=%s',
            ...
            if _coded_value_comparator(
                ...
                self._logger.debug(
                    'instantiating "cancel audio pause" operation from existing descriptor handle=%s',
                    operation_descriptor_container.Handle,
                )
    ...
    def _set_global_audio_pause(self, params: ExecuteParameters) -> ExecuteResult:
        ...
        with self._mdib.alert_state_transaction() as mgr:
            for as_entity in alert_system_entities:
                ...
                audible_signals = [
                    ssa
                    for ssa in as_entity.state.SystemSignalActivation
                    if ssa.Manifestation == pm_types.AlertSignalManifestation.AUD
                ]
                active_audible_signals = [
                    ssa for ssa in audible_signals if ssa.State != pm_types.AlertActivation.PAUSED
                ]
                if len(active_audible_signals) > 0:
                    for ssa in active_audible_signals:
                        ssa.State = pm_types.AlertActivation.PAUSED
                    self._logger.info('SetAudioPauseOperation: set alert system "%s" to paused', as_entity.handle)
                    ...
                    audible_child_alert_signals = [
                        d
                        for d in child_alert_signals
                        if d.descriptor.Manifestation == pm_types.AlertSignalManifestation.AUD
                    ]
        ...
        with self._mdib.alert_state_transaction() as mgr:
            ...
            for as_entity in alert_system_entities:
                ...
                if len(paused_audible_signals) > 0:
                    ...
                    # handle all audible alert signals of this alert system
                    all_alert_signal_entities = self._mdib.entities.by_node_type(pm_names.AlertSignalDescriptor)
                    child_alert_signals = [e for e in all_alert_signal_entities if e.parent_handle == as_entity.handle]
                    audible_child_alert_signals = [
                        d
                        for d in child_alert_signals
                        if d.descriptor.Manifestation == pm_types.AlertSignalManifestation.AUD
...
class AudioPauseProvider(GenericAudioPauseProvider):
    ...
    def make_missing_operations(self, sco: AbstractScoOperationsRegistry) -> list[OperationDefinitionBase]:
        ...
        if not self._set_global_audio_pause_operations:
            self._logger.debug(
                'adding "set audio pause" operation, no descriptor in mdib (looked for code = %s)',
                NomenclatureCodes.MDC_OP_SET_ALL_ALARMS_AUDIO_PAUSE,
            )
            set_ap_operation = activate_op_cls(
                'AP__ON',
                mds_entity.handle,
                self._set_global_audio_pause,
                coded_value=NomenclatureCodes.MDC_OP_SET_ALL_ALARMS_AUDIO_PAUSE,
            )
            self._set_global_audio_pause_operations.append(set_ap_operation)
            ops.append(set_ap_operation)
        if not self._cancel_global_audio_pause_operations:
            self._logger.debug(
                'adding "cancel audio pause" operation, no descriptor in mdib (looked for code = %s)',
                NomenclatureCodes.MDC_OP_SET_CANCEL_ALARMS_AUDIO_PAUSE,
            )
            cancel_ap_operation = activate_op_cls(
                'AP__CANCEL',
                mds_entity.handle,
                self._cancel_global_audio_pause,
                coded_value=NomenclatureCodes.MDC_OP_SET_CANCEL_ALARMS_AUDIO_PAUSE,
            )
