...
from tutorial.productandroles.waveformprovider.realtimesamples import Annotator, RtSampleArray
...
if TYPE_CHECKING:
    ...
    from sdc11073.provider.protocols.waveformprotocol import AnnotatorProtocol, WaveformGeneratorProtocol
...
class _SampleArrayGenerator:
    ...
    def __init__(self, model: AbstractDataModel, descriptor_handle: str, generator: WaveformGeneratorProtocol):
        self._model = model
...
class GenericWaveformProvider:
    ...
    def register_waveform_generator(self, descriptor_handle: str, wf_generator: WaveformGeneratorProtocol):
        ...
        :param wf_generator: a waveform generator instance
        """
    ...
    def _log_waveform_timing(self, behind_schedule_seconds: float):
        ...
        if last_logged_delay >= self.WARN_LIMIT_REALTIMESAMPLES_BEHIND_SCHEDULE > behind_schedule_seconds:
            self._logger.info(
                'RealTimeSampleTimer delay is back inside limit of %2f seconds (mdib version=%d)',
                self.WARN_LIMIT_REALTIMESAMPLES_BEHIND_SCHEDULE,
            ...
            self._logger.warning(
                'RealTimeSampleTimer is %.4f seconds behind schedule (mdib version=%d)',
                behind_schedule_seconds,
                self._mdib.mdib_version,
            )
            self._last_logged_delay = behind_schedule_seconds
            self._last_log_time = now
