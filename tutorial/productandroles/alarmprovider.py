...
class AlertDelegateProvider(RoleProvider):
    ...
    def make_operation_instance(
        ...
        if pm_names.SetAlertStateOperationDescriptor == operation_descriptor_container.NODETYPE:
            if (
                pm_names.AlertSignalDescriptor == op_target_entity.node_type
                ...
                if (
                    ...
                    self._logger.debug(
                        '%s: added handler "self._delegate_alert_signal" for %s target=%s',
    ...
    def _delegate_alert_signal(self, params: ExecuteParameters) -> ExecuteResult:
        ...
        self._logger.info(
            'delegate alert signal %s of %s from %s to %s',
            operation_target_handle,
            op_target_entity.state,
            op_target_entity.state.ActivationState,
        ...
        if op_target_entity.descriptor.SignalDelegationSupported:
            if value.ActivationState == pm_types.AlertActivation.ON:
                modified = self._pause_fallback_alert_signals(op_target_entity, all_alert_signal_entities)
            else:
                modified = self._activate_fallback_alert_signals(op_target_entity, all_alert_signal_entities)
    ...
    def _on_timeout_delegate_alert_signal(self, operation_instance: OperationDefinitionProtocol):
        ...
        all_alert_signal_entities = self._mdib.entities.by_node_type(pm_names.AlertSignalDescriptor)
        self._logger.info(
            'timeout alert signal delegate operation=%s target=%s',
            operation_instance.handle,
            operation_target_handle,
        )
    ...
    def _pause_fallback_alert_signals(
        self,
        ...
        for fallback_entity in self._get_fallback_signals(delegable_signal_entity, all_signal_entities):
    ...
    def _get_fallback_signals(
        ...
        return [
            tmp
            for tmp in all_signal_entities
            if not tmp.descriptor.SignalDelegationSupported
...
class AlertSystemStateMaintainer(RoleProvider):
    ...
    def _update_alert_system_states(self, alert_system_entities: Iterable[EntityProtocol]):
        ...
        for alert_system_entity in alert_system_entities:
            ...
            all_tech_entities = [
                d for d in all_alert_condition_entities if d.descriptor.Kind == pm_types.AlertConditionKind.TECHNICAL
            ]
            ...
            all_phys_entities = [
                d
                for d in all_alert_condition_entities
            ...
            alert_system_entity.state.PresentPhysiologicalAlarmConditions = [
                e.handle for e in all_present_phys_entities
            ]
            alert_system_entity.state.LastSelfCheck = time.time()
            alert_system_entity.state.SelfCheckCount = (
                1 if alert_system_entity.state.SelfCheckCount is None else alert_system_entity.state.SelfCheckCount + 1
            )


class AlertPreCommitHandler(RoleProvider):
    """Provide some generic alarm handling functionality.
    ...
    def on_pre_commit(self, mdib: ProviderMdibProtocol, transaction: StateTransactionManagerProtocol):
        ...
        for changed_alert_condition_state in changed_alert_condition_states:
            self._update_alert_signals(changed_alert_condition_state, all_alert_signal_entities, mdib, transaction)

        # find all alert systems for changed_alert_condition_states
        alert_system_states = self._find_alert_systems_with_modifications(transaction, changed_alert_condition_states)
    ...
    @staticmethod
    def _update_alert_system_states(
        ...
        for _alert_system_state in alert_system_states:
            ...
            alert_system_state.PresentPhysiologicalAlarmConditions = [
                s.DescriptorHandle for s in all_present_phys_states
            ]
            if write_entity:
                transaction.write_entity(alert_system_state)

    @staticmethod
    def _get_changed_alert_condition_states(
        transaction: StateTransactionManagerProtocol,
    ) -> list[AbstractStateProtocol]:
    ...
    @staticmethod
    def _update_alert_signals(
        changed_alert_condition: AbstractStateProtocol,
        ...
        my_alert_signal_entities = [
            e
            for e in all_alert_signal_entities
        ...
        local_alert_signal_entities = [
            a for a in my_alert_signal_entities if not a.descriptor.SignalDelegationSupported
        ]
        ...
        lookup = {
            (True, True): (pm_types.AlertActivation.PAUSED, pm_types.AlertSignalPresence.OFF),
    ...
    def _find_alert_systems_with_modifications(
        self,
        transaction: StateTransactionManagerProtocol,
