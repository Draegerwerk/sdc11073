...
class GenericContextProvider(RoleProvider):
        ...
        with self._mdib.context_state_transaction() as mgr:
            for proposed_st in proposed_context_states:
                ...
                if old_state_container is None:
                    ...
                    self._logger.info(
                        'new %s, DescriptorHandle=%s Handle=%s',
                        proposed_st.NODETYPE.localname,
                    ...
                    if (
                        old_state_container.ContextAssociation == pm_types.ContextAssociation.ASSOCIATED
                        ...
                        proposed_st.BindingMdibVersion = mgr.new_mdib_version
                        proposed_st.BindingStartTime = time.time()
                        handles = self._mdib.xtra.disassociate_all(
                            entity,
...
class GenericPatientContextProvider(GenericContextProvider):
    ...
    def make_operation_instance(
        ...
        if (
            self._patient_context_entity is not None
            and operation_descriptor_container.OperationTarget == self._patient_context_entity.handle
        ):
            pc_operation = self._mk_operation_from_operation_descriptor(
                operation_descriptor_container,
                operation_cls_getter,
                operation_handler=self._set_context_state,
            )
