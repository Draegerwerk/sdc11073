...
from sdc11073.loghelper import get_logger_adapter, basic_logging_setup
...
from sdc11073.pysoap.soapclient import Fault
from sdc11073.pysoap.soapclient import SoapClient
from sdc11073.wsdiscovery.wsdimpl import Service
from sdc11073.xml_types import wsd_types, pm_types, eventing_types
...
class DiscoProxyClient:
    def __init__(self,
        ...
        self._soap_client = SoapClient(disco_proxy_address,
                                       socket_timeout=5,
        ...
        self._msg_converter = MessageConverterMiddleware(
            message_reader, message_factory, self._logger, self)
    ...
    def stop(self, unsubscribe=False):
    ...
    def search_services(self,
                        ...
                        scopes: wsd_types.ScopesType | None = None) -> list[Service]:
        ...
        for probe_match in probe_response.ProbeMatch:
            service = Service(types=probe_match.Types,
    ...
    def send_resolve(self, epr) -> wsd_types.ResolveMatchesType:
        payload = wsd_types.ResolveType()
        payload.EndpointReference.Address = epr
        inf = HeaderInformationBlock(action=payload.action, addr_to=ADDRESS_ALL)
        created_message = _mk_wsd_soap_message(inf, payload)
        received_message = self._soap_client.post_message_to('', created_message)
        resolve_response = wsd_types.ResolveMatchesType.from_node(received_message.p_msg.msg_node)
    ...
    def send_subscribe(self):
        ...
        if response_action == EventingActions.SubscribeResponse:
            ...
            self._logger.error(  # noqa: PLE1205
                'subscribe: Fault  response : {}', fault)
    ...
    def send_unsubscribe(self):
        ...
        if response_action == EventingActions.UnsubscribeResponse:
            ...
            self._logger.error(  # noqa: PLE1205
                'unsubscribe: Fault  response : {}', fault)
    ...
    def _send_bye(self, service: Service):
        ...
        created_message = _mk_wsd_soap_message(inf, bye)
        received_message = self._soap_client.post_message_to('', created_message)
        # hello_response = wsd_types.HelloType.from_node(received_message.p_msg.msg_node)
    ...
    def on_post(self, request_data: RequestData) -> CreatedMessage:
        ...
        if request_data.message_data.action == wsd_types.HelloType.action:
            ...
            service = Service(types=hello.Types,
                              scopes=hello.Scopes,
                              x_addrs=hello.XAddrs,
                              epr=hello.EndpointReference.Address,
                              instance_id='',  # Todo: needed in any way?
    ...
    def on_get(self, request_data: RequestData) -> CreatedMessage:
        print('on_get')
        return EmptyResponse()
