...
class TestDeviceWaveform(unittest.TestCase):
    ...
    def test_waveformGeneratorHandling(self):
        tr = waveforms.TriangleGenerator(
            min_value=0, max_value=10, waveformperiod=2.0, sampleperiod=0.005
        )
        st = waveforms.SawtoothGenerator(
            min_value=0, max_value=10, waveformperiod=2.0, sampleperiod=0.01
        )
        si = waveforms.SinusGenerator(
            min_value=-8.0, max_value=10.0, waveformperiod=5.0, sampleperiod=0.05
        )
        ...
        for h in HANDLES:
            ...
            self.assertEqual(
                rt_sample_array.activation_state, pmtypes.ComponentActivation.ON
            )
        ...
        for h in HANDLES:
            ...
            self.assertTrue(
                expectedCount - 5 <= len(rt_sample_array.samples) <= expectedCount + 5
            )  #
    ...
    def test_waveformSubscription(self):
        ...
        tr = waveforms.TriangleGenerator(
            min_value=0, max_value=10, waveformperiod=2.0, sampleperiod=0.02
        )
        st = waveforms.SawtoothGenerator(
            min_value=0, max_value=10, waveformperiod=2.0, sampleperiod=0.02
        )
        si = waveforms.SinusGenerator(
            min_value=-8.0, max_value=10.0, waveformperiod=5.0, sampleperiod=0.02
        )
        ...
        self.mdib.registerAnnotationGenerator(
            annotation,
        ...
        self.sdc_device = sdc11073.sdcdevice.SdcDevice(
            self.wsDiscovery,
            uuid,
            self._model,
            self._device,
        ...
        test_subscr = mockstuff.TestDevSubscription(
            self.sdc_device.mdib.sdc_definitions.Actions.Waveform, nbr_reports_flag=21
        )
        self.sdc_device.subscriptionsManager._subscriptions.addObject(test_subscr)
