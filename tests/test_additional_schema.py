...
prefix_namespace_foo = PrefixNamespace(
    'foo',
    'http://test/foo',
...
class TestAdditionalSchema(unittest.TestCase):
    def setUp(self):
        ...
        self.provider = SomeDevice.from_mdib_file(
            wsdiscovery=None,
            epr=uuid.uuid4(),
            components=provider_components,
            mdib_xml_path='mdib_tns.xml',
        )
        self.consumer = SdcConsumer(
            'http://127.0.0.1:10000',  # exact value does not matter
    ...
    def test_consumer(self):
        ...
        It is sufficient to test only one of the validators, because they are all the same in
        msg_factory and msg_reader of consumer and provider.
        """
        self.consumer.msg_reader.read_received_message(mdib_data.format('abcs').encode('utf-8'))  # correct attribute
        self.assertRaises(
            ValidationError,
            self.consumer.msg_reader.read_received_message,
