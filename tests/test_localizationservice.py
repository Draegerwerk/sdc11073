...
class TestLocationService(unittest.TestCase):
    def setUp(self):
        ...
        for ref in self.ref_list:
            for lang, version, width, lines in product(
                self.lang_list,
                self.versions_list,
                self.width_list,
                self.lines_list,
                ...
                text = ref if lines in (None, 1) else '\n'.join([ref] * lines)
    ...
    def test_no_filter(self):
        ...
        self.assertEqual(
            len(texts),
            len(self.ref_list) * len(self.lang_list) * len(self.width_list) * len(self.lines_list),
        )  # one version per ref and language
        for t in texts:
