...
class TestDeviceServices(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        self.wsd.stop()
        print(f'############### tearDown {self._testMethodName} done ##############')

    def _mk_get_request(self, sdc_device: SdcProvider, port_type: str, method: str, path: str) -> CreatedMessage:
        name_space = sdc_device.mdib.sdc_definitions.ActionsNamespace
        nsm = self.sdc_device.mdib.nsmapper  # shortcut

        action = f'{name_space}/{port_type}/{method}'
    ...
    def test_dispatch(self):
        ...
        response = sub_dispatcher.do_post(
            http_header,
            path,
            peer_name,
            self.sdc_device.msg_factory.serialize_message(get_env),
        )
        _code, _reason, response_string = response
        ...
        get_env = self._mk_get_request(
            self.sdc_device,
            context_service.port_type_name.localname,
            'GetContextStates',
            path,
        )
        http_header = {}
    ...
    def test_change_alarm_prio(self):
        get_service = self.sdc_device.hosted_services.get_service
        path = '123'
        with self.sdc_device.mdib.descriptor_transaction() as tr:
            alarm_condition_descriptor = tr.get_descriptor('0xD3C00109')
            alarm_condition_descriptor.Priority = AlertConditionPriority.LOW
        get_env = self._mk_get_request(self.sdc_device, get_service.port_type_name.localname, 'GetMdDescription', path)
        http_header = {}
        request = RequestData(http_header, path, 'foo')
        request.message_data = self.msg_reader.read_received_message(
    ...
    def test_get_context_states(self):
        ...
        query = '{}[@{}="{}"]'.format(
            _ns.MSG.doc_name('ContextState'),
            _ns.XSI.doc_name('type'),
            _ns.PM.doc_name('LocationContextState'),
        )
        location_ontext_nodes = response.p_msg.payload_element.xpath(query, namespaces=_ns.ns_map)
        self.assertEqual(len(location_ontext_nodes), 1)
        identification_node = location_ontext_nodes[0].find(pm.Identification)
        self.assertEqual(
            identification_node.get('Extension'),
            f'{loc.fac}/{loc.bldng}/{loc.flr}/{loc.poc}/{loc.rm}/{loc.bed}',
        )

        location_detail_node = location_ontext_nodes[0].find(pm.LocationDetail)
        self.assertEqual(location_detail_node.get('PoC'), loc.poc)
        self.assertEqual(location_detail_node.get('Bed'), loc.bed)
        self.assertEqual(location_detail_node.get('Facility'), loc.fac)
    ...
    def test_wsdl(self):
        ...
        for hosted in dev.hosted_services.dpws_hosted_services.values():
            ...
            for _src in (inputs, outputs):
                for i in inputs:
                    action_keys = [k for k in i.attrib if k.endswith('Action')]
