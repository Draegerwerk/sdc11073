...
class TestDeviceSubscriptions(unittest.TestCase):
    def setUp(self):
        """validate test data"""
        self.mdib = sdc11073.mdib.DeviceMdibContainer.fromMdibFile(
            os.path.join(mdibFolder, "70041_MDIB_Final.xml")
        )
        ...
        mdib_d10 = sdc11073.mdib.DeviceMdibContainer.fromMdibFile(
            os.path.join(mdibFolder, "70041_MDIB_Final.xml")
        )
        self.sdcDevice_d10 = sdc11073.sdcdevice.SdcDevice(
            self.wsDiscovery,
            my_uuid,
            self._model,
            self._device,
    ...
    def _verify_proper_namespaces(self, report):
        ...
        for ns in (
            namespaces.Prefix_Namespace.PM.namespace,
    ...
    def test_waveformSubscription(self):
        for sdcDevice in self._allDevices:
            test_subscr = mockstuff.TestDevSubscription(
                sdcDevice.mdib.sdc_definitions.Actions.Waveform, nbr_reports_flag=21
            )
            sdcDevice.subscriptionsManager._subscriptions.addObject(test_subscr)

            tr = waveforms.TriangleGenerator(
                min_value=0, max_value=10, waveformperiod=2.0, sampleperiod=0.01
            )
            st = waveforms.SawtoothGenerator(
                min_value=0, max_value=10, waveformperiod=2.0, sampleperiod=0.01
            )
            si = waveforms.SinusGenerator(
                min_value=-8.0, max_value=10.0, waveformperiod=5.0, sampleperiod=0.01
            )
            ...
            self.assertTrue(test_subscr.reports_event.wait(timeout=10))
            self.assertGreater(len(test_subscr.reports), 20)
            report = test_subscr.reports[-1]  # a
    ...
    def test_episodicMetricReportSubscription(self):
        ...
        hosted = sdc11073.pysoap.soapenvelope.DPWSHosted(
            endpointReferencesList=[
                sdc11073.pysoap.soapenvelope.WsaEndpointReferenceType(
                    "http://1.2.3.4:6000"
                )
            ],
            typesList=["Get"],
        ...
        for sdcDevice in self._allDevices:
            clSubscr = sdc11073.sdcclient.subscription.ClSubscription(
                dpwsHosted=hosted,
            ...
            subscrRequest = clSubscr._mkSubscribeEnvelope(
                subscribe_epr="http://otherdevice:123/bla", expire_minutes=59
            )
            ...
            sdcDevice.subscriptionsManager.soapClients["localhost:123"] = (
                DummySoapClient()
            )
            response = sdcDevice.subscriptionsManager.onSubscribeRequest(
                httpHeader,
            ...
            clSubscr._handleSubscribeResponse(
                ReceivedSoap12Envelope.fromXMLString(response.as_xml())
            )
            ...
            self.assertEqual(
                devSubscr.notifyRefNodes[0].text, clSubscr.notifyTo_identifier.text
            )
            self.assertEqual(devSubscr.endToAddress, endTo)
            self.assertEqual(
                devSubscr.endToRefNodes[0].text, clSubscr.end_to_identifier.text
            )

            # verify that client subscription object contains the subscription identifier of the device Subscription object
            self.assertEqual(
                clSubscr.dev_reference_param[0].tag, devSubscr.my_identifier.tag
            )
            self.assertEqual(
                clSubscr.dev_reference_param[0].text, devSubscr.my_identifier.text
            )
    ...
    def test_episodicMetricReportEvent(self):
        ...
        for sdcDevice in self._allDevices:
            ...
            with sdcDevice.mdib.mdibUpdateTransaction() as mgr:
                ...
                st.Validity = "Vld"
            self.assertEqual(len(test_subscr.reports), 1)
            response = test_subscr.reports[0]
            self._verify_proper_namespaces(response)
            print(response.as_xml(pretty=True))
            response.validate_envelope(sdcDevice._handler.xml_validator)
            ...
            idents = env.headerNode.findall(test_subscr.IDENT_TAG)
    ...
    def test_episodicContextReportEvent(self):
        ...
        for sdcDevice in self._allDevices:
            test_subscr = mockstuff.TestDevSubscription(
                sdcDevice.mdib.sdc_definitions.Actions.EpisodicContextReport
            )
            sdcDevice.subscriptionsManager._subscriptions.addObject(test_subscr)
            patientContextDescriptor = sdcDevice.mdib.descriptions.NODETYPE.getOne(
    ...
    def test_notifyOperation(self):
        for sdcDevice in self._allDevices:
            test_subscr = mockstuff.TestDevSubscription(
                sdcDevice.mdib.sdc_definitions.Actions.OperationInvokedReport
            )
            sdcDevice.subscriptionsManager._subscriptions.addObject(test_subscr)
            sdcDevice.subscriptionsManager.notifyOperation(
                MdibVersionGroup(1234, "seq", 12),
                transactionId=123,
    ...
    def test_invalid_GetStatus_Renew(self):
        ...
        for sdcDevice in self._allDevices:
            ...
            subscrRequest = clSubscr._mkSubscribeEnvelope(
                subscribe_epr="http://otherdevice/bla:123", expire_minutes=59
            )
            subscrRequest.validate_envelope(sdcDevice._handler.xml_validator)
    ...
    def test_periodicMetricReportSubscription(self):
        ...
        for sdcDevice in self._allDevices:
            clSubscr = sdc11073.sdcclient.subscription.ClSubscription(
                dpwsHosted=hosted,
                actions=[sdcDevice.mdib.sdc_definitions.Actions.PeriodicMetricReport],
                notification_url=notifyTo,
                endTo_url=endTo,
                ident="",
                xml_validator=None,
            ...
            subscrRequest.validate_envelope(sdcDevice._handler.xml_validator)
            (subscrRequest.as_xml(pretty=True))
    ...
    def test_periodicMetricReportEvent(self):
        ...
        for sdcDevice in self._allDevices:
            ...
            self.assertTrue(testPeriodicSubscr.reports_event.wait(timeout=5))
