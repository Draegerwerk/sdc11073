...
MDIB_FOLDER = Path(__file__).parent
MDIB_TWO_MDS_PATH = MDIB_FOLDER / 'mdib_two_mds.xml'
MDIB_TNS_PATH = MDIB_FOLDER / 'mdib_tns.xml'
...
class TestMdibTransaction(unittest.TestCase):
    def setUp(self):
        self.mdib = ProviderMdib.from_mdib_file(MDIB_TWO_MDS_PATH, protocol_definition=definitions_sdc.SdcV1Definitions)
    ...
    def test_activate_operation_argument(self):
            ...
            <pm:MdDescription DescriptionVersion="8">
                <pm:Mds Handle="mds0" DescriptorVersion="5" SafetyClassification="MedA">
                    <pm:Sco Handle="Sco.mds0" DescriptorVersion="2">
                        <pm:Operation {1} xsi:type="pm:ActivateOperationDescriptor" Handle="SVO.38.3569"
                        DescriptorVersion="2" SafetyClassification="MedA" OperationTarget="3569"
                        MaxTimeToFinish="PT00H00M02S" Retriggerable="false">
                            <pm:Type Code="193821">
            <pm:ConceptDescription Lang="en-US">An operation to cancel global all audio pause</pm:ConceptDescription>
                            </pm:Type>
                            <pm:Argument>
        ...
        mdibs = [
            TestData(
                mdib_text=mdib_dummy.format('', '', delaration.format(my_prefix), f'{my_prefix}:'),
            ...
            TestData(
                mdib_text=mdib_dummy.format('', '', delaration.format(xsd_prefix), f'{xsd_prefix}:'),
                expected_qname=expected_qname_xsd,
            ),
            TestData(
                mdib_text=mdib_dummy.format('', '', 'xmlns="http://www.w3.org/2001/XMLSchema"', ''),
                expected_qname=expected_qname_xsd,
            ),
            TestData(
                mdib_text=mdib_dummy.format('', '', delaration_any_uri.format(xsd_prefix), f'{xsd_prefix}:'),
        ...
        for test_data in mdibs:
            ...
            mdib_node, _ = mdib_container.reconstruct_mdib_with_context_states()
            arg_nodes = mdib_node.xpath(
                '//*/pm:Arg',
                namespaces={'pm': 'http://standards.ieee.org/downloads/11073/11073-10207-2017/participant'},
            )
