"""Implementation of the SDC ContextService port type."""
...
class ContextService(ServiceWithOperations):
    """Provider port type for the SDC ContextService."""

    port_type_name = PrefixesEnum.SDC.tag('ContextService')
    WSDLMessageDescriptions = (
        WSDLMessageDescription('SetContextState', (f'{msg_prefix}:SetContextState',)),
        WSDLMessageDescription('SetContextStateResponse', (f'{msg_prefix}:SetContextStateResponse',)),
        WSDLMessageDescription('GetContextStates', (f'{msg_prefix}:GetContextStates',)),
        WSDLMessageDescription('GetContextStatesResponse', (f'{msg_prefix}:GetContextStatesResponse',)),
        WSDLMessageDescription('EpisodicContextReport', (f'{msg_prefix}:EpisodicContextReport',)),
        WSDLMessageDescription('PeriodicContextReport', (f'{msg_prefix}:PeriodicContextReport',)),
    )
    WSDLOperationBindings = (
        # TODO: generate wsdl:fault  # noqa: FIX002, TD002, TD003
        WSDLOperationBinding('SetContextState', 'literal', 'literal'),
    ...
    def register_hosting_service(self, hosting_service: DPWSHostedService):
        """Register the SetContextState and GetContextStates post handlers in the hosting service."""
        ...
        hosting_service.register_post_handler(
            DispatchKey(actions.SetContextState, msg_names.SetContextState), self._on_set_context_state
        )
        hosting_service.register_post_handler(
            DispatchKey(actions.GetContextStates, msg_names.GetContextStates), self._on_get_context_states
        )

    def _on_set_context_state(self, request_data: RequestData) -> CreatedMessage:
        """Handle a SetContextState request and return the operation response message."""
    ...
    def _on_get_context_states(self, request_data: RequestData) -> CreatedMessage:  # noqa: C901
        """Handle a GetContextStates request and return the matching context states as a response message."""
        data_model = self._sdc_definitions.data_model
        ...
        if len(requested_handles) > 0:
            self._logger.info('_on_get_context_states requested Handles:{}', requested_handles)  # noqa: PLE1205
        with self._mdib.mdib_lock:
            if len(requested_handles) == 0:
                ...
                for handle in requested_handles:
                    ...
                    if not tmp:
                        # R5042: If a HANDLE reference from the msg:GetContextStates/msg:HandleRef list does match
                        # an MDS descriptor, then all context states that are part of this MDS SHALL be included in
                        # the result list.
    ...
    def add_wsdl_port_type(self, parent_node: xml_utils.LxmlElement) -> NoReturn:
        """Add the ContextService wsdl:portType node with its operations to parent_node."""
    ...
    def send_episodic_context_report(self, states: list[AbstractStateContainer], mdib_version_group: MdibVersionGroup):
        """Build an EpisodicContextReport from the given states and send it to all subscribers."""
        data_model = self._sdc_definitions.data_model
        ...
        self._logger.debug('sending episodic context report {}', states)  # noqa: PLE1205
        subscription_mgr.send_to_subscribers(body_node, report.action.value, mdib_version_group)

    def send_periodic_context_report(
        self, periodic_states_list: list[PeriodicStates], mdib_version_group: MdibVersionGroup
    ):
        """Build a PeriodicContextReport from the buffered states and send it to all subscribers."""
        data_model = self._sdc_definitions.data_model
        subscription_mgr = self.hosting_service.subscriptions_manager
        ...
        self._logger.debug(  # noqa: PLE1205
            'sending periodic context report, contains last {} episodic updates', len(periodic_states_list)
        )
