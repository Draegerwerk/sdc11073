...
class AbstractStateContainer(ContainerBase):
    ...
    def updateFromOtherContainer(self, other, skippedProperties=None):
        if other.__class__ != self.__class__:
            raise RuntimeError('Update from a node with different type is not possible! Have "{}", got "{}"'.format(
...
class AbstractDeviceComponentStateContainer(AbstractStateContainer):
    ...
    CalibrationInfo = cp.SubElementProperty([domTag('CalibrationInfo')], valueClass=pmtypes.T_CalibrationType)
    NextCalibration = cp.SubElementProperty([domTag('NextCalibration')], valueClass=pmtypes.T_CalibrationType)
    PhysicalConnector = cp.SubElementProperty([domTag('PhysicalConnector')],
                                              valueClass=pmtypes.PhysicalConnectorInfo)  # optional
...
class BatteryStateContainer(AbstractDeviceComponentStateContainer):
    ...
    _props = ('CapacityRemaining', 'Voltage', 'Current', 'Temperature', 'RemainingBatteryTime', 'ChargeStatus',
              'ChargeCycles')
...
class LimitAlertConditionStateContainer(AlertConditionStateContainer):
    ...
    MonitoredAlertLimits = cp.NodeAttributeProperty('MonitoredAlertLimits',
                                                    defaultPyValue=pmtypes.AlertConditionMonitoredLimits.ALL_OFF)  # required, pm:AlertConditionMonitoredLimits
...
class AbstractContextStateContainer(AbstractMultiStateContainer):
    ...
    ContextAssociation = cp.NodeAttributeProperty('ContextAssociation',
...
class PatientContextStateContainer(AbstractContextStateContainer):
    ...
    _props = ('Givenname', 'Middlename', 'Familyname', 'Birthname', 'Title', 'Sex', 'PatientType', 'DateOfBirth',
              'Height', 'Weight', 'Race')
