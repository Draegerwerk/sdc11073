...
from .dpws_types import (
    DeviceMetadataDialectURI,
    DeviceRelationshipTypeURI,
    HostedServiceType,
    HostServiceType,
    ThisDeviceType,
    ThisModelType,
...
class ThisModelMetadataSection(PropertyBasedPMType):
    """ThisModel metadata section."""

    MetadataReference = cp.SubElementProperty(dpws_tag('ThisModel'), value_class=ThisModelType)
    Location = cp.AnyUriTextElement(wsx_tag('Location'), is_optional=True)
    Dialect = cp.AnyURIAttributeProperty(
        'Dialect', default_py_value=DeviceMetadataDialectURI.THIS_MODEL, is_optional=False
    )
...
class ThisDeviceMetadataSection(PropertyBasedPMType):
    """ThisDevice metadata section."""

    MetadataReference = cp.SubElementProperty(dpws_tag('ThisDevice'), value_class=ThisDeviceType)
    Location = cp.AnyUriTextElement(wsx_tag('Location'), is_optional=True)
    Dialect = cp.AnyURIAttributeProperty(
        'Dialect', default_py_value=DeviceMetadataDialectURI.THIS_DEVICE, is_optional=False
    )
    Identifier = cp.AnyURIAttributeProperty('Identifier')
    _props = ('MetadataReference', 'Location', 'Dialect', 'Identifier')
...
class RelationshipMetadataSection(PropertyBasedPMType):
    """Metadata section for relationship metadata."""

    MetadataReference = cp.SubElementProperty(
        dpws_tag('Relationship'), value_class=MetaDataRelationship, default_py_value=MetaDataRelationship()
    )
    Location = cp.AnyUriTextElement(wsx_tag('Location'), is_optional=True)
    Dialect = cp.AnyURIAttributeProperty(
        'Dialect', default_py_value=DeviceMetadataDialectURI.RELATIONSHIP, is_optional=False
    )
    Identifier = cp.AnyURIAttributeProperty('Identifier')
    _props = ('MetadataReference', 'Location', 'Dialect', 'Identifier')
...
class LocationMetadataSection(PropertyBasedPMType):
    """Metadata section for location metadata."""

    Location = cp.AnyUriTextElement(wsx_tag('Location'), is_optional=True)
    Dialect = cp.AnyURIAttributeProperty(
        'Dialect', default_py_value=default_ns_helper.WSDL.namespace, is_optional=False
    )
    Identifier = cp.AnyURIAttributeProperty('Identifier')
...
dialect_lookup = {
    DeviceMetadataDialectURI.THIS_MODEL: (ThisModelMetadataSection, 'this_model', 'MetadataReference'),
    DeviceMetadataDialectURI.THIS_DEVICE: (ThisDeviceMetadataSection, 'this_device', 'MetadataReference'),
    DeviceMetadataDialectURI.RELATIONSHIP: (RelationshipMetadataSection, 'relationship', 'MetadataReference'),
    default_ns_helper.WSDL.namespace: (LocationMetadataSection, 'wsdl_location', 'Location'),
}


class Metadata(MessageType):
    """Metadata exchange Metadata class."""

    NODETYPE = wsx_tag('Metadata')
    action = f'{default_ns_helper.WXF.namespace}/GetResponse'
    MetadataSection = cp.SubElementListProperty(wsx_tag('MetadataSection'), value_class=PropertyBasedPMType)
    ...
    def from_node(cls, node):  # noqa: ANN001, ANN206
        ...
        for section_node in node[0]:
            ...
            section_cls, member_name, section_member_name = dialect_lookup.get(dialect, (None, None, None))
